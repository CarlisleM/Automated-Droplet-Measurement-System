...
if strcmp(videoOrientation,'landscape') == 1    % Video is in landscape view
    for i=1:totalFrameCounter-1 % Number of frames - 1
                    ...
                    if onlyCountOnce == 0
                        if dropletTrackingX(i+1,v+1) - dropletTrackingX(i,v) > 0 & dropletTrackingX(i+1,v+1) - dropletTrackingX(i,v) < mean(nonzeros(dropletSeparation(i,:)))*1.5
                           % disp('pair meets criteria');
                            ...
                            fopen(sprintf('Results_Jetting/Velocity/droplet_%i.txt',dropletCount),'wt'); % Create new droplet txt file
                        end
                    end
                end
            end
        ...
        for k = 1:length(currentDroplets)-1
            dropletFileID = fopen(sprintf('Results_Jetting/Velocity/droplet_%i.txt',currentDroplets(length(currentDroplets)+1-k)),'a');
            try
...
frequencyResultsID = fopen(strcat('Results_Velocity_',currentFileName,'/velocity_results.txt'),'wt');
fprintf(frequencyResultsID,'Number of droplets detected: %i\t Average Velocity: %.3f\t Velocity standard deviation: %.3f\t', dropletCount, avgVelocity*pixelRatio, stdevVelocity*pixelRatio);
