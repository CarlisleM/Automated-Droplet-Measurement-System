% Count number of droplets and write deformation results to the corresponding droplet text files
...
if strcmp(videoOrientation,'landscape') == 1    % Video is in landscape view
    for i = 1:totalFrameCounter-1 % Number of frames - 1
                    ...
                    if onlyCountOnce == 0
                        if dropletTrackingX(i+1,v+1) - dropletTrackingX(i,v) > 0 & dropletTrackingX(i+1,v+1) - dropletTrackingX(i,v) < mean(nonzeros(dropletSeparation(i,:)))*1.5
                           % disp('pair meets criteria');
                            ...
                            fopen(strcat('Results_Deformation_',currentFileName,sprintf('/droplet_%i.txt',dropletCount)),'wt'); % Create new droplet txt file
                        end
                    end
                end
            end
        ...
        % Append droplet deformation to appropriate droplet txt file
        for k = 1:length(currentDroplets)
            dropletFileID = fopen(strcat('Results_Deformation_',currentFileName,sprintf('/droplet_%i.txt',currentDroplets(length(currentDroplets)+1-k))),'a');
            fprintf(dropletFileID,'%.5f\t', dropletDeformation(i,k)*pixelRatio);
        end
...
% Get droplet deformation average and standard deviation
avgDeformation = mean(nonzeros(dropletDeformation));
stdevDeformation = std(nonzeros(dropletDeformation));

% Create txt file containing number of droplets, average deformation, and deformation stdev
deformationResultsID = fopen(strcat('Results_Deformation_',currentFileName,'/deformation_results.txt'),'wt');
fprintf(deformationResultsID,'Number of droplets detected: %i\t Average deformation: %.5f\t Deformation standard deviation: %.5f\t', dropletCount, avgDeformation*pixelRatio, stdevDeformation*pixelRatio);
