...
if strcmp(videoOrientation,'landscape') == 1    % Video is in landscape view
    for i=1:totalFrameCounter-1 % Number of frames - 1
                    ...
                    if onlyCountOnce == 0
                        if dropletTrackingX(i+1,v+1) - dropletTrackingX(i,v) > 0 & dropletTrackingX(i+1,v+1) - dropletTrackingX(i,v) < mean(nonzeros(dropletSeparation(i,:)))*1.5
                           % disp('pair meets criteria');
                            ...
                            fopen(strcat('Results_Jetting_',currentFileName,'/Frequency',sprintf('/droplet_%i.txt',dropletCount)),'wt'); % Create new droplet txt file
                        end
                    end
                end
            end
        ...
        for k = 1:length(currentDroplets)-1
            dropletFileID = fopen(strcat('Results_Jetting_',currentFileName,'/Frequency',sprintf('/droplet_%i.txt',currentDroplets(length(currentDroplets)+1-k))),'a');
            try
...
frequencyResultsID = fopen(strcat('Results_Jetting_',currentFileName,'/Frequency','/frequency_results.txt'),'wt');
