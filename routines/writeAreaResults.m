% Count number of droplets and write area results to the corresponding droplet text files
...
if strcmp(videoOrientation,'landscape') == 1    % Video is in landscape view
    for i=1:totalFrameCounter-1 % Number of frames - 1
                    ...
                    if onlyCountOnce == 0
                        if dropletTrackingX(i+1,v+1) - dropletTrackingX(i,v) > 0 & dropletTrackingX(i+1,v+1) - dropletTrackingX(i,v) < mean(nonzeros(dropletSeparation(i,:)))*1.5
                           % disp('pair meets criteria');
                            ...
                            fopen(strcat('Results_Area_',currentFileName,sprintf('/droplet_%i.txt',dropletCount)),'wt'); % Create new droplet txt file 
                        end
                    end
                end
            end
        ...
        % Append droplet area to appropriate droplet txt file
        for k = 1:length(currentDroplets)
            dropletFileID = fopen(strcat('Results_Area_',currentFileName,sprintf('/droplet_%i.txt',currentDroplets(length(currentDroplets)+1-k))),'a');
            fprintf(dropletFileID,'%.3f\t', dropletArea(i,k)*pixelRatio);
        end
                    ...
                    if onlyCountOnce == 0
                        if dropletTrackingY(i+1,v+1) - dropletTrackingY(i,v) > 0 & dropletTrackingY(i+1,v+1) - dropletTrackingY(i,v) < mean(nonzeros(dropletSeparation(i,:)))*1.5
                           % disp('pair meets criteria');
                            ...
                            onlyCountOnce = 1;
                            dropletCount = dropletCount+1;  % Increase droplet count
                            fopen(strcat('Results_Area_',currentFileName,sprintf('/droplet_%i.txt',dropletCount)),'wt');
                        end
                    end
                end
            end
...
% Get droplet area average and standard deviation
avgArea = mean(nonzeros(dropletArea));
stdevArea = std(nonzeros(dropletArea));

% Create txt file containing number of droplets, average area, and area stdev
areaResultsID = fopen(strcat('Results_Area_',currentFileName,'/area_results.txt'),'wt');
fprintf(areaResultsID,'Number of droplets detected: %i\t Average area: %.3f\t Area standard deviation: %.3f\t', dropletCount, avgArea*pixelRatio, stdevArea*pixelRatio);
