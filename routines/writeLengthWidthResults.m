...
if strcmp(videoOrientation,'landscape') == 1    % Video is in landscape view
    for i=1:totalFrameCounter-1 % Number of frames - 1
                    ...
                    if onlyCountOnce == 0
                        if dropletTrackingX(i+1,v+1) - dropletTrackingX(i,v) > 0 & dropletTrackingX(i+1,v+1) - dropletTrackingX(i,v) < mean(nonzeros(dropletSeparation(i,:)))*1.5
                           % disp('pair meets criteria');
                            ...
                            fopen(strcat('Results_LengthWidth_',currentFileName,sprintf('/droplet_%i.txt',dropletCount)),'wt'); % Create new droplet txt file
                        end
                    end
                end
            end
        ...
        for k = 1:length(currentDroplets)
            dropletFileID = fopen(strcat('Results_LengthWidth_',currentFileName,sprintf('/droplet_%i.txt',currentDroplets(length(currentDroplets)+1-k))),'a');
...
lengthWidthResultsID = fopen(strcat('Results_LengthWidth_',currentFileName,'/length_width_results.txt'),'wt');
fprintf(lengthWidthResultsID,'Number of droplets detected: %i\t Average length: %.3f\t Length standard deviation: %.3f\t Average width: %.3f\t Width standard deviation: %.3f\t', dropletCount, avgLength*pixelRatio, stdevLength*pixelRatio, avgWidth*pixelRatio, stdevWidth*pixelRatio);
